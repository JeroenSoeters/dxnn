...
%The create function first generates 3 weights, with the 3rd weight being the Bias. The Neuron is spawned first, and is sent the PIds of the Sensor and Actuator that it's connected with. Then the Cortex element is registered and provided with the PIds of all the elements in the NN system.
...
%After the neuron finishes setting its SPId and APId to that of the Sensor and Actuator respectively, it starts waiting for the incoming signals. The neuron expects a vector of length 2 as input, and sas soon as the input arrives, the neuron processes the signal and passes the output vector to the outgoing APId.
	...
	dot([],[Bias],Acc) ->
		Acc + Bias.
%The dot function takes a product of two vectors, it can operate on a weight vector with and without a bias. When there is no bias in the weight list, both the Input ve tor and the Weight vector are of the same length. When Bias is present, then when the Input list empties out, the Weights list still has 1 value remaining, it's Bias.
...
%The actuator waits for a control signal coming from a Neuron. As soon as the signal arrives, the actuator executes it's function, pts/1, which prints the value to the screen.
